...
const DistinguishingMarks = () => {
  ...
  const GetDistinguishingMarksApi = async () => {
    ...
    try {
      ...
      // Determine the number of birth marks available in the API response
      const existingBirthMarksCount = Object.keys(apiData)
        .filter((key) => key.startsWith("birthMark") && apiData[key])
        .length;

      // Update visibleBirthMarks to show existing marks
    ...
    <div className="px-2 my-3 overflow-hidden">
      ...
      <div className="space-y-3">
        {[...Array(visibleBirthMarks)].map((_, index) => (
          <input
            key={index}
            className="flex flex-col gap-2 w-1/3 my-3 py-1 px-3 border rounded-md border-gray-300 placeholder:text-black placeholder:text-sm"
            type="text"
