...
const Ethnicity = () => {
  ...
  // Define default ethnicity options
  ...
  // Function to fetch the ethnicity data from API
  const GetEthnicityApi = async () => {
    ...
    try {
      ...
      if (response?.data?.isData) {
        ...
        setData({ ...filteredData }); // Include ethnicityDetailId and exclude unwanted fields
      }
    } catch (error) {
  ...
  const renderEthnicityCheckboxes = () => {
    ...
    // Use default options if no data is available
