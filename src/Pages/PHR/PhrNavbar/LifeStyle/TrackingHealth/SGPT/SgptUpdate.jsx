...
const SgptUpdate = () => {
  ...
  const [sgptId, setSgpt] = useState(location.state?.SGPTId || null);
  console.log("sgptId", sgptId);
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post(
        "https://service.healthcapita.com/api/PHR/SavePhrSgpt",
        formData
      );
        ...
        <div className="py-4 px-4 sm:px-6 md:px-4 lg:px-12">
            ...
            <form className=" p-4 w-[50%] ">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
                <div className="flex flex-col gap-1">
                  <label className="font-semibold">SGPT Level</label>
                  <input
                    type="number"
                    name="sgptlevel"
                    value={formData.sgptlevel}
          ...
          {sgptId && (
            <MedicationAndreminders
              ...
              recordId={sgptId}
              recordIdKey={"sgptid"}
              fetchDataUrl="https://service.healthcapita.com/api/PHR/GetphrPrescribedMedicationsSgptId"
              saveUrl="https://service.healthcapita.com/api/PHR/SavephrSgptPrescribedMedications"
              editUrl="https://service.healthcapita.com/api/PHR/GetPhrSgptPrescribedMedication?prescribedMedicationsId"
              deleteUrl="https://service.healthcapita.com/api/PHR/DeletePhrSgptPrescribedMedication?prescribedMedicationsId"
            />
          )}
        </div>
      </div>
