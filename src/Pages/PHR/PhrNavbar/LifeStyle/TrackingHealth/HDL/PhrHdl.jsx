...
const PhrHdl = () => {
  ...
  const openHdlAddPage = () => {
    window.scrollTo({ top: 0, behavior: "smooth" });
    navigate("/PhrHdlUpdatePage");
  };

  const openHdlUpdatePage = (HDLId) => {
    window.scrollTo({ top: 0, behavior: "smooth" });
    navigate("/PhrHdlUpdatePage", { state: { HDLId } });
  };
  const userId = 10;
  ...
  const [showHdlForm, setHdlForm] = useState(false);
  const [selectedContact, setSelectedConatct] = useState({});

  const openHdlForm = async (HDLId) => {
    setHdlForm(true);
    try {
      const response = await axios.get(
        `https://service.healthcapita.com/api/PHR/GetPhrHdlById/${HDLId}/${userId}`
      );
      console.log("responsedataaaa", response?.data?.response);
  ...
  useEffect(() => {
    const getApiData = async () => {
      try {
        ...
        // console.log("hdl", response?.data?.data);
      ...
      <div className="py-3 px-2 bg-[#F9FAFB] rounded-md">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
            <img
              className="p-2 rounded-full bg-[#12B76A]"
              src={PhrAssets.Hdl}
              alt=""
            />
            <p className="font-medium leading-5 text-lg text-[#111928]">HDL</p>
          </div>
          <AddBtn onClick={openHdlAddPage} />
        </div>
                ...
                <tr key={item.id}>
                  <td className="pl-1 py-4 text-sm text-gray-900 border-b cursor-pointer">
                    <img
                      ...
                      onClick={() => openHdlForm(item.HDLId)}
                      alt="Options"
                      className="w-6 h-6"
                    />
                  </td>
                  ...
                  <td className="px-4 py-4 text-base text-gray-900 border-b">
                    <div className="flex gap-4 items-center">
                      <button onClick={() => openHdlUpdatePage(item.HDLId)}>
                        <img src={PhrAssets.Edit} alt="" />
                      </button>

                      <button
                        className="flex gap-1 items-center  font-semibold"
                        onClick={() => handleDelete(item.HDLId)}
                      >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-[50%] relative overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
            <div className="pt-2 px-6 mx-auto pb-10">
              <div className="flex justify-between items-center">
                <h2 className="font-semibold text-2xl">HDL Information</h2>
                <img
                  src={PhrAssets.Close}
                  alt=""
                  onClick={closeHdlForm}
              ...
              <div className="grid sm:grid-cols-3 gap-8 mb-4">
                <div className="flex flex-col gap-1">
                  <p>HDL Level</p>
                  <p className="font-semibold">{selectedContact.hdllevel}</p>
                </div>
