...
const TriglyceridesUpdate = () => {
  ...
  const [triglycerides, setTriglycerides] = useState(
    location.state?.BloodTriglyceridesId || null
  );
  console.log("triglycerides", triglycerides);
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post(
        "https://service.healthcapita.com/api/PHR/SavePhrTriGlyCeride",
        formData
      );
      console.log("triglycerides", response);
      if (response?.data?.status) {
        setTriglycerides(response?.data?.bloodTriglyceridesId);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  useEffect(() => {
    if (triglycerides) {
      axios
        .get(
          `https://service.healthcapita.com/api/PHR/GetPhrTriglyceridesById/${triglycerides}/${userId}`
        )

        .then((response) => {
          console.log("responseeeeeeeeee", response?.data?.data);
          if (response?.data?.status) {
    ...
    <>
      <div className="min-h-screen bg-white flex flex-col">
        ...
        <PhrProtectwithPassword
          ...
          isDisplayed={formData.isDisplayUnderSummayPage}
        ...
        <div className="py-4 px-4 sm:px-6 md:px-4 lg:px-12">
            ...
            <form className=" p-4 w-[50%] ">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div className="flex flex-col gap-1">
                  <label className="font-semibold">Triglycerides Level</label>
                  <input
                    type="number"
                    name="bloodTriglyceridesLevel"
                    value={formData.bloodTriglyceridesLevel}
                ...
                <div className="grid sm:grid-cols-2 gap-5 col-span-3">
                  <div className="flex flex-col gap-1">
          ...
          {triglycerides && (
            <MedicationAndreminders
              ...
              recordId={triglycerides}
              recordIdKey={"bloodTriglyceridesId"}
              fetchDataUrl="https://service.healthcapita.com/api/PHR/GetphrPrescribedMedicationsTriglycerideId"
              saveUrl="https://service.healthcapita.com/api/PHR/SavephrTriglyceridePrescribedMedications"
              editUrl="https://service.healthcapita.com/api/PHR/GetPhrTriglyceridesPrescribedMedication?prescribedMedicationsId"
              deleteUrl="https://service.healthcapita.com/api/PHR/DeletePhrTriglyceridesPrescribedMedication?prescribedMedicationsId"
            />
          )}
        </div>
      </div>
...
export default TriglyceridesUpdate;
