...
const WeightUpdate = () => {
  ...
  const [weightId, setWeightId] = useState(location.state?.WeightId || null);
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post(
        "https://service.healthcapita.com/api/PHR/SavePhrWeight",
        formData
      );
      console.log("weightttttttt", response);
      if (response?.data?.status) {
        setWeightId(response?.data?.weightId);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  useEffect(() => {
    if (weightId) {
      axios
        ...
        .then((response) => {
          console.log("weightEdit", response?.data?.status);
          if (response?.data?.status) {
        ...
        <div className="py-4 px-4 sm:px-6 md:px-4 lg:px-12">
            ...
            <form className=" p-4 w-[50%] ">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div className="flex flex-col gap-1">
                  <label className="font-semibold">Weight (Kgs)</label>
                  <input
                    type="number"
                    name="kiloGram"
          ...
          {weightId && (
            <MedicationAndreminders
              ...
              recordId={weightId}
              recordIdKey={"weightId"}
              fetchDataUrl="https://service.healthcapita.com/api/PHR/GetphrPrescribedMedicationsWeightId"
              saveUrl="https://service.healthcapita.com/api/PHR/SavephrWeightPrescribedMedications"
              editUrl="https://service.healthcapita.com/api/PHR/GetPhrWeightPrescribedMedication?prescribedMedicationsId"
              deleteUrl="https://service.healthcapita.com/api/PHR/DeletePhrWeightPrescribedMedication?prescribedMedicationsId"
            />
          )}
        </div>
      </div>
