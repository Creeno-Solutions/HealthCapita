...
const InsuranceDetails = () => {
  ...
  const data = [
    { id: 1, icon: PhrAssets.ThreeDotted, email: "john@example.com", role: "Admin" },
    { id: 2, icon: PhrAssets.ThreeDotted, email: "jane@example.com", role: "Editor" },
  ];
  ...
  return (
    <>
      <div className="py-5 px-5 my-7 bg-[#F9FAFB]">
            ...
            <tbody>
              {data.map((item) => (
                <tr key={item.id} className="hover:bg-gray-100">
                  ...
                  <img src={item.icon}  onClick={() => toggleDropdown(item.id)} alt="Options" className="w-6 h-6" />
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-900 border-b">
                  --
                  </td>
