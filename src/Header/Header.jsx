...
const Navigate =  useNavigate()
  return (
    <header className="bg-[#001940] py-4 px-4 sm:px-6 md:px-8 lg:px-12 md:py-4 lg:py-1 text-[#9CA3AF] sticky top-0 z-50">
    <div className="flex justify-between items-center">
      <div className="flex items-center gap-6 md:gap-8">
        <img src={HomeAssets.logo} alt="Logo" className="h-8 sm:h-12"/>
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        `relative cursor-pointer after:content-[''] after:h-1 after:bg-white after:absolute after:left-0 after:-bottom-2.5 after:w-full after:border-none transition-all duration-300 ${
          isActive ? "after:opacity-100 text-white" : "after:opacity-0 hover:after:opacity-100 hover:text-gray-300"
        }`
      }
    >
      <li>Dashboard</li>
    ...
    >
      <li>Device Data</li>
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          />
          <button onClick={()=>Navigate('/')} className="text-xs sm:text-sm md:text-base font-medium">
            Logout
          </button>
      ...
      {isMenuOpen && (
        <nav className="md:hidden">
          <ul className="flex flex-col text-center gap-5 mt-5 sm:mt-4 text-xs sm:text-sm text-gray-200">
            ...
            <li className="hover:text-gray-300 cursor-pointer">Device Data</li>
          </ul>
        </nav>
      )}
    </header>
